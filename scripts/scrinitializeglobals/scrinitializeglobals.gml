...
function scrInitializeGlobals() {
	...
	global.autosave = false;        //keeps track of whether or not to autosave the next timer the player is created
	...
	global.controllerDelay = -1;    //handles delay between switching between keyboard/controller so that the player can't use both at the same timer
