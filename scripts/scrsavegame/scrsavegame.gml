...
/// @param saveposition
///saves the game
///argument0 - sets whether the game should save the player's current location or just save the deaths/timer
function scrSaveGame(argument0) {

	var savePosition = argument0;

	//save the player's current location variables if the script is currently set to (we don't want to save the player's location if we're just updating death/timer)
	if (savePosition)
	{    
	...
	ds_map_add(saveMap,"timer",global.timer);
